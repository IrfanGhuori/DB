...
class DB
{
    ...
    public function update($table_name, $fields = [], $id = null)
    {
        ...
        if (isset($id)) {
            ...
            if (is_numeric(value: $id)) {
                ...
                $this->bindValues[] = $id;
                // if there is an Array
            } elseif (is_array(value: $id)) {
                $arr = $id;
                $count_arr = count(value: $arr);
                $x = 0;
                ...
                foreach ($arr as $param) {
                    ...
                    if ($count_param == 1) {
                        ...
                        $operators = explode(separator: ',', string: "=,>,<,>=,>=,<>");
                        $operatorFound = false;

                        foreach ($operators as $operator) {
                            if (strpos(haystack: $param[0], needle: $operator) !== false) {
                                $operatorFound = true;
                                break;
                            }
                        }
                        ...
                        if ($operatorFound) {
                            ...
                            $this->where .= "`" . trim(string: $param[0]) . "` = ?";
                        }

                        $this->bindValues[] = $param[1];
                    } elseif ($count_param == 3) {
                        $this->where .= "`" . trim(string: $param[0]) . "` " . $param[1] . " ?";
                        $this->bindValues[] = $param[2];
                    }
            ...
            $this->getSQL = $this->sql;
            $stmt = $this->dbh->prepare(query: $this->sql);
            $stmt->execute(params: $this->bindValues);
            return $stmt->rowCount();
        }
        // end if there is an ID or Array
        // $this->getSQL = "<b>Attention:</b> This Query will update all rows in the table, luckily it didn't execute yet!, use exec() method to execute the following query :<br>". $this->sql;
    ...
    public function insert($table_name, $fields = [])
    {
        ...
        $keys = implode(separator: '`, `', array: array_keys($fields));
        $values = '';
        $x = 1;
        foreach ($fields as $field => $value) {
            $values .= '?';
            $this->bindValues[] = $value;
            if ($x < count(value: $fields)) {
                $values .= ', ';
            }
            $x++;
        }
    ...
    {
        $columns = explode(separator: ',', string: $columns);
        foreach ($columns as $key => $column) {
            $columns[$key] = trim(string: $column);
        }

        $columns = implode(separator: '`, `', array: $columns);
    ...
    public function where()
    {
        ...
        if ($num_args == 1) {
            ...
            $operatorFound = false;
            foreach ($operators as $operator) {
                if (strpos(haystack: $args[0], needle: $operator) !== false) {
                    $operatorFound = true;
                    break;
                }
            }
            ...
            $this->where .= "`" . trim(string: $args[0]) . "` " . $args[1] . " ?";
    ...
    public function get()
    {
        ...
        $x = 0;
        foreach ($rows as $key => $row) {
            $collection->offsetSet(offset: $x++, value: $row);
        }
    ...
    public function orderBy($field_name, $order = 'ASC')
    {
        ...
        $order = trim(string: strtoupper(string: $order));
    ...
    public function paginate($page, $limit)
    {
        ...
        $stmt = $this->dbh->prepare(query: $countSQL);
        $stmt->execute(params: $this->bindValues);
        $totalRows = $stmt->fetch(mode: PDO::FETCH_NUM)[0];
        // echo $totalRows;

        $offset = ($page - 1) * $limit;
        // Refresh Pagination Array
        $this->pagination['currentPage'] = $page;
        $this->pagination['lastPage'] = ceil(num: $totalRows / $limit);
        ...
        $stmt = $this->dbh->prepare(query: $sql);
        $stmt->execute(params: $this->bindValues);
        $this->rowCount = $stmt->rowCount();
    ...
    public function count()
    {
        ...
        return $stmt->fetch(mode: PDO::FETCH_NUM)[0];
    }
...
class MareiCollection implements ArrayAccess
{
    ...
    public function item($key): mixed
    {
